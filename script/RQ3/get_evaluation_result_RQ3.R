...
cvae.prediction.dir = "/doc2/wuchen/DeepLineDP-master1.1/DeepLineDP-master1.0/output/prediction/DeepLineDP/within-release/"
ros.prediction.dir = "/doc2/wuchen/experiment/RQ4/DeepLineDP-master/output/prediction/DeepLineDP/within-release/"
adasyn.prediction.dir = "/doc2/wuchen/experiment/RQ4/adasyn/DeepLineDP/within-release/"
borderlinesmote.prediction.dir = "/doc2//wuchen/experiment/RQ4/borderlinesmote/DeepLineDP/within-release/"

all_files = list.files(prediction_dir)
original_all_files = list.files(original_prediction_dir)
cvae_all_files= list.files(cvae.prediction.dir)
ros_all_files= list.files(ros.prediction.dir)
adasyn_all_files= list.files(adasyn.prediction.dir )
borderlinesmote_all_files= list.files(borderlinesmote.prediction.dir)
...
for(f in cvae_all_files)
{
  cvae_df <- read.csv(paste0(cvae.prediction.dir, f))
  cvae_df_all <- rbind(cvae_df_all, cvae_df)
}

ros_df_all <- NULL

for(f in ros_all_files)
{
  ros_df <- read.csv(paste0(ros.prediction.dir, f))
  ros_df_all <- rbind(ros_df_all, ros_df)
}

adasyn_df_all <- NULL

for(f in adasyn_all_files)
{
  adasyn_df <- read.csv(paste0(adasyn.prediction.dir, f))
  adasyn_df_all <- rbind(adasyn_df_all, adasyn_df)
}

borderlinesmote_df_all <- NULL

for(f in borderlinesmote_all_files)
{
  borderlinesmote_df <- read.csv(paste0(borderlinesmote.prediction.dir , f))
  borderlinesmote_df_all <- rbind(borderlinesmote_df_all, borderlinesmote_df)
}
print("RQ1")
# ---------------- Code for RQ1 -----------------------#
...
cvae.result = get.file.level.eval.result(cvae.prediction.dir, "CVAE")
print("CVAE1")
ros.result = get.file.level.eval.result(ros.prediction.dir, "ROS")
print("CVAE1")
adasyn.result = get.file.level.eval.result(adasyn.prediction.dir, "ADASYNSMOTE")
print("CVAE1")
borderlinesmote.result = get.file.level.eval.result(borderlinesmote.prediction.dir, "BorderlineSMOTE")
print("CVAE1")
...
all.result = rbind(gcn.result, baseline.result,cvae.result,ros.result,adasyn.result,borderlinesmote.result )
...
custom_order <- c("EU-LLDP", "DeepLineDP", "CVAE", "ROS","ADASYNSMOTE","BorderlineSMOTE")  
...
cvae.line.result <- calculate_metrics(cvae_df_all)
ros.line.result <- calculate_metrics(ros_df_all)
adasyn.line.result <- calculate_metrics(adasyn_df_all)
borderlinesmote.line.result <- calculate_metrics(borderlinesmote_df_all)
...
names(cvae.line.result) = c("IFA", "Recall20%LOC", "Effort@20%Recall")
names(ros.line.result) = c("IFA", "Recall20%LOC", "Effort@20%Recall")
names(adasyn.line.result) = c("IFA", "Recall20%LOC", "Effort@20%Recall")
names(borderlinesmote.line.result) = c("IFA", "Recall20%LOC", "Effort@20%Recall")
...
cvae.line.result$technique = 'CVAE'
ros.line.result$technique = 'ROS'
adasyn.line.result$technique = 'ADASYNSMOTE'
borderlinesmote.line.result$technique = 'BorderlineSMOTE'



all.line.result = rbind(deepline.dp.line.result, baseline.line.result,cvae.line.result,ros.line.result,adasyn.line.result,borderlinesmote.line.result)
